...
# == Globals ==
# # # # #
_VERSION = "0.2.2 alpha"
# to do list, make rss items append to top, pop off bottom, instead of vice versa like now. will break feed for one run, probably
...
Global Options:
	...
	scanMins: An integer option. Default 15. Values are in minutes. The number of minutes between scans. If a feed uses the <ttl> tag, it will be respected. That is, if you have scanMins set to 10 and the site sets <ttl>900</ttl> (900 seconds; 15 mins); then the feed will be scanned every other time. 
	...
	rssFeed: Boolean Option. Default False. Setting this option allows you to create your own rss feed of the objects you have downloaded. It's a basic feed, likely to not include links to the original files. The keys are (all are required):
	rssLength: Integer. Default 20. An integer. How many entries should the RSS feed store before it starts dropping old items. 0 would literally mean to store no items
	rssTitle: A string. Default "some RSS Title".  The title the rss feed will carry.
	rssLink: A string: Default 'somelink.com/%s' % self['rssFilename']. Where the rss feed can be located. Typically an http link.
	rssDescription: A string. Default "Some RSS Description". A short description of what the feed contains.
	rssFilename: A string. Default 'rssdownloadfeed.xml'. Where to store the feed on disk.
...
Thread options:
	...
	checkTime<x>Day: A string option. Either the standard 3 letter abbreviation of the day of the week, or the full name. One of Three options that will specify a scan time. the <x> is an integer. Will only scan the rss feed during the day specified. Can Further curtail scan time with Start and Stop (see next).
	checkTime<x>Start: An integer option. Default: 0. The hour (0-23) at which to start scanning on correlated day. MUST specify checkTime<x>Day.
	checkTime<x>Stop: An integer option. Default 23. The hour (0-23) at which to stop scanning on correlated day. MUST specify checkTime<x>Day.
	...
	download<x>: No default. where <x> is an integer, this is a 'positive' hit regex. This is required for download<x>true and download<x>false.
	download<x>False: Default = True. However, this is not strictly a boolean option. True means you want to keep regExFalse against download<x>. If not, set to False, and there will be no 'negative' regex that will be checked against. You can also set this to a string (i.e. a regex) that will be a negative regex ONLY for the corresponding download<x>. Most strings are legal, however the following False/True/Yes/No/0/1 are reserved words when used alone and are interpreted, in a case insensitive manner as Boolean arguments. Requires a corresponding download<x> argument.
	download<x>True. A Boolean option. default True. True checks against regExTrue. False ignores regExTrue. Requires a corresponding download<x> argument.
	download<x>Dir. A String option. Default None. If specified, the first of the download<x> tuples to match up with the download name, downloads the file to the directory specified here. Full path is recommended.
	"""
	...
	try: 
		if url:
			...
			for i in xrange( 2, len( a[2:] ) ):
				if a[i] and ' ' in a[i]: a[i] = urllib.quote( a[i] )
			url = urlparse.urlunparse( tuple( a ) )
	except:
		pass
	if not cj:
		if os.path.isfile(getConfig(filename=configFile)['global']['cookieFile']):
...
def downloadFile(url, threadName, rssItemNode, downloadLDir):
	try: data = mechRetrievePage(url)
		...
		sys.stdout.write('error grabbing url%s' % os.linesep)
		return False
	dataPage = data.read()
	dataInfo = data.info()
...
class GlobalOptions(UserDict):
	...
	def __init__(self, verbose=True, downloadDir=os.getcwd(), runOnce=False, minSize=None, maxSize=None, log=False, logFile='downloads.log', saveFile='savedstate.dat', scanMins=15, lockPort=8023, cookieFile='cookies.txt', workingDir=os.getcwd(), daemonInfo = 'daemon.info', rssFeed=False):
...
##	else:
	...
	if saved.failedDown:
		...
		for i in  xrange( len( saved.failedDown) - 1, -1, -1 ):
			#failureUrl, failureName, failureRss, failureDir  = i
